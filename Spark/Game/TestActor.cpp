...
void TestActor::OnTick(float _dt)
{
	const Vec2 movement
	{
		static_cast<float>(IsKeyDown(KEY_E)) - static_cast<float>(IsKeyDown(KEY_Q)),
	...
	const float rotation = static_cast<float>(IsKeyDown(KEY_D)) - static_cast<float>(IsKeyDown(KEY_A));
